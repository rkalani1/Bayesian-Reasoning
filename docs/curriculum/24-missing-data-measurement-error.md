...
!!! note "Mathematical Detail"
    Pattern-mixture and selection models are reparameterizations of the same joint. Selection: \(p(Y \mid X)\, p(R \mid Y, X)\). Pattern-mixture: \(p(Y \mid R, X)\, p(R \mid X)\). Identification always fails without a restriction that reaches into the missing pattern. A typical restriction is a shift: \(\operatorname{logit} P(Y \le k \mid R = 0, X) = \operatorname{logit} P(Y \le k \mid R = 1, X) + \delta_k\). The sensitivity parameter \(\delta_k\) is not estimable from the observed data. You either elicit it (“how many steps worse do we fear the missing are?”) or you report a curve of posteriors against \(\delta\).
