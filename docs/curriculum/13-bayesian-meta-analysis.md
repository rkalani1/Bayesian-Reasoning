...
\(\mu\) is the mean of the distribution from which study-specific true effects are drawn. \(\tau\) is the between-study standard deviation on the same scale as \(y_i\). A fixed-effect (common-effect) analysis is the special case \(\tau = 0\). Frequentist random-effects methods (DerSimonian–Laird, REML) estimate \(\tau\) and then, often, pretend the estimate was known — the Hartung–Knapp adjustment being the partial exception, built to widen the interval for exactly this reason. With three late-window EVT studies that pretense is the whole problem. \(\tau\) is barely identified. Its point estimate is a noisy function of three residuals. Plug-in intervals for \(\mu\) are too narrow, and the prediction interval for a new \(\theta_{\text{new}}\) is fantasy.
...
Teaching sketch, not a retrieval. Add an imagined unpublished study D: log-OR \(0.05\), SE \(0.55\), \(n \approx 80\), broader imaging, never submitted because the interval crossed 1. Under the half-normal \(\tau\) prior already on the table, including D as if it had been found drops the posterior median of \(\mu\) (from about 0.85 to about 0.73) and pushes the lower end of the prediction interval for \(\theta_{\text{new}}\) further toward harm (from about −0.4 to about −0.5) — a move a hub without trial-grade imaging should notice. That is the direction of the bias you are trying to bound. It is not a number to put in a forest plot labeled “adjusted.”
